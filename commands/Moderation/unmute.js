...
exports.run = (client, message, args) => {
 ...
 if(!message.guild.me.hasPermission("MANAGE_MESSAGES"))
 {
  ...
  if (modlogchannel === null) {
   ...
   let reason = args[1]
   db.set(`mute_${message.guild.id}:${member.id}`, 'false');
	message.channel.send({embed: { color: 0xFFFFFF, title: "Success!", description: `${user.username} has been unmuted!`} });
		   const embed = new Discord.RichEmbed()
    .setColor(0xFF0000)
    .setTimestamp()
    .setTitle(`<:sadaf:548227551370280971> **Unmute**`)
